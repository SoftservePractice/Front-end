...
function Detail() {
  ...
  async function AddData() {
    ...
    if (validate()) {
      const { model, vendorCode, description, compatibleVehicles, category } =
        editData;
      const result = await addData(
        `${link}/Detail?model=${model}&vendorCode=${vendorCode}&description=${description}&compatibleVehicles=${compatibleVehicles}&catId=${category}`
      );
      console.log(result);
  ...
  async function UpdateData() {
    ...
    if (validate()) {
      ...
      const result = await updateData(
        `${link}/detail/${id}?model=${model}&vendorCode=${vendorCode}&description=${description}&compatibleVehicles=${compatibleVehicles}$category=${category}`
      );
      if (result) {
        const newData = [...data];
  ...
  return (
    <div className="content">
      {modalVisible && (
        <div className="content__modal">
          <div className="content__block-modal">
            ...
            <p style={{ margin: 5 }}>Vendor Code</p>
            <input
              placeholder="Vendor Code..."
              value={editData.vendorCode}
              className={
                !validity.vendorCode ? "main-input-invalid" : "main-input"
              }
              onChange={(e) =>
            ...
            <input
              ...
              className={
                !validity.description ? "main-input-invalid" : "main-input"
              }
              onChange={(e) =>
            ...
            <p style={{ margin: 5 }}>Compatible Vehicles</p>
            <input
              placeholder="Compatible Vehicles..."
              value={editData.compatibleVehicles}
              className={
                !validity.compatibleVehicles
                  ? "main-input-invalid"
                  : "main-input"
              }
              onChange={(e) =>
            ...
            <p style={{ margin: 5 }}>Category ID</p>
            <input
              type={"number"}
              placeholder="Category ID..."
              value={editData.category}
              className={
                !validity.category ? "main-input-invalid" : "main-input"
              }
              onChange={(e) =>
            ...
            {!editData.id ? (
              ...
              <button
                className="content__update-btn-modal main-btn"
                onClick={UpdateData}
              >
                Update
              </button>
      ...
      <div className="content__block-main main-block">
        {!data ? (
          <span className="table__no-connect">No detail found</span>
        ) : (
