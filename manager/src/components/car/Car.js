...
function Car() {
  ...
  async function AddData() {
    ...
    if (validate()) {
      ...
      const result = await addData(
        `${link}/car?mark=${mark}&year=${year}&vin=${vin}&carNumber=${carNumber}&clientId=${client}`
      );
  ...
  async function UpdateData() {
    ...
    if (validate()) {
      ...
      const result = await updateData(
        `${link}/car/${id}?mark=${mark}&year=${year}&vin=${vin}&carNumber=${carNumber}&client=${client}`
      );
      if (result) {
        const newData = [...data];
  ...
  return (
    <div className={"content"}>
      {modalVisible && (
        <div className="content__modal">
          <div className="content__block-modal">
            ...
            <p style={{ margin: 5 }}>Mark</p>
            <input
              placeholder="Mark..."
              value={editData.mark}
              className={!validity.mark ? "main-input-invalid" : "main-input"}
              onChange={(e) =>
                setEditData({ ...editData, mark: e.target.value })
              }
            ></input>

            <p style={{ margin: 5 }}>Year</p>
            <input
              placeholder="Year..."
              value={editData.year}
            ...
            <p style={{ margin: 5 }}>Car Number</p>
            <input
              placeholder="Car Number..."
              value={editData.carNumber}
              className={
                !validity.carNumber ? "main-input-invalid" : "main-input"
              }
              onChange={(e) =>
      ...
      {!data ? (
        <span className="table__no-connect">No category found</span>
      ) : (
        <div className="content__block-main">
          <table className="table">
            ...
            {data
              .filter((item) =>
                item.mark.toLowerCase().includes(filterModel.toLowerCase())
              )
              .map((item) => {
                return (
                  <tr key={item.id}>
