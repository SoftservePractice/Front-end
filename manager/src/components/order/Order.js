...
function Order() {
  ...
  async function GetData() {
    const result = await getAllData(
      "http://egorhi-001-site1.htempurl.com/order"
    );
    setData(result);
  }
  //ДОБАВЛЕНИЕ
  async function AddNewData() {
    ...
    if (validate()) {
      ...
      const result = await addData(
        `http://egorhi-001-site1.htempurl.com/order?clientId=${client}&technician=${technician}&start=${start}&end=${end}&finalPrice=${finalPrice}&car=${car}&carMileage=${carMileage}&appointmentTime=${appointmentTime}`
      );
      setData([...data, result]);
  ...
  async function RemoveData(id) {
    const result = await removeData(
      `http://egorhi-001-site1.htempurl.com/order/${id}`
    );
    if (result) {
  ...
  async function UpdateData() {
    ...
    if (validate()) {
      ...
      const result = await updateData(
        `http://egorhi-001-site1.htempurl.com/order/${id}?client=${client}&technician=${technician}&start=${start}&end=${end}&finalPrice=${finalPrice}&car=${car}&carMileage=${carMileage}&appointmentTime=${appointmentTime}`
      );
      if (result) {
        const newData = [...data];
  ...
  return (
    <div className="content">
      {modalVisible && (
        <div className="content__modal">
          <div className="content__block-modal">
            <button className="content__cancel-btn-modal" onClick={Cancel}>
              {" "}
              X
            </button>

            <p style={{ margin: 5 }}>Client ID</p>
            <input
            ...
            <p style={{ margin: 5 }}>Technician ID</p>
            <input
              placeholder="Technician ID..."
              type={"number"}
              value={editData.technician}
              className={
                !validity.technician ? "main-input-invalid" : "main-input"
              }
              onChange={(e) =>
            ...
            <p style={{ margin: 5 }}>Final Price</p>
            <input
              placeholder="Final Price..."
              type={"number"}
              value={editData.finalPrice}
              className={
                !validity.finalPrice ? "main-input-invalid" : "main-input"
              }
              onChange={(e) =>
            ...
            <p style={{ margin: 5 }}>Car Mileage</p>
            <input
              placeholder="Car Mileage..."
              type={"number"}
              value={editData.carMileage}
              className={
                !validity.carMileage ? "main-input-invalid" : "main-input"
              }
              onChange={(e) =>
            ...
            <p style={{ margin: 5 }}>Appointment Time</p>
            <input
              placeholder="Appointment Time..."
              type={"date"}
              value={editData.appointmentTime}
              className={
                !validity.appointmentTime ? "main-input-invalid" : "main-input"
              }
              onChange={(e) =>
            ...
            {!editData.id ? (
              <button
                className="content__add-btn-modal main-btn"
                onClick={AddNewData}
              >
                Add
              </button>
      ...
      {!data ? (
        <span className="table__no-connect">No work found</span>
      ) : (
